                    ...
                    <div class="mt-16">
                        <form action="{{ route('soldierprofile.update', $profile->id) }}" method="post" class="space-y-4" enctype="multipart/form-data">
                            ...
                            <div>
                                ...
                                <select disabled name="number_type" id="number_type" class="mt-1 p-2 w-full border rounded-md" required onchange="toggleInputField()">
                                ...
                                <div id="inputField" style="@php echo ($profile->number_type === 'Only number' || $profile->number_type === 'BJO-number') ? 'display:block;' : 'display:none;'; @endphp">
                                    <input disabled value="@php if($profile->number_type != null ) echo $profile->number; @endphp" placeholder="" type="text" name="number" id="number" class="mt-1 p-2 w-full border rounded-md" required>
                                </div>
                                
                            </div>
                            <div>
                                <label for="rank" class="mt-2 block text-sm font-medium text-gray-700">পদবী</label>
                                <select disabled name="rank" id="rank" class="mt-1 p-2 w-full border rounded-md" required>
                            ...
                            <div>
                                ...
                                <select disabled name="trade" id="trade" class="mt-1 p-2 w-full border rounded-md" required>
                            ...
                            <div>
                                ...
                                <input disabled value ="{{$profile->name}}" type="text" name="name" id="name" class="mt-1 p-2 w-full border rounded-md">
                            </div>
                            ...
                            <div>
                                ...
                                <select disabled name="coy" id="coy" class="mt-1 p-2 w-full border rounded-md" required>
                            ...
                            <div>
                                ...
                                <select disabled name="marital_status" id="marital_status" class="mt-1 p-2 w-full border rounded-md" required>
                            ...
                            <div>
                                ...
                                <select  disabled name="blood_gp" id="blood_gp" class="mt-1 p-2 w-full border rounded-md" required>
                            ...
                            <div class="flex">
                                <div class="mr-2">
                                    ...
                                    <input disabled value ="{{$profile->height_feet  }}" placeholder="ফুট (সংখ্যা)" type="text" name="height_feet" id="height_feet" class="mt-1 p-2 w-full border rounded-md" required>
                                </div>
                                <div>
                                    <label for="height_inch" class="mt-2 block text-sm font-medium text-gray-700">উচ্চতা (ইঞ্চি)</label>
                                    <input disabled value ="{{$profile->height_inch}}" placeholder="ইঞ্চি (সংখ্যা) " type="text" name="height_inch" id="height_inch" class="mt-1 p-2 w-full border rounded-md" required>
                                </div>
                            </div>
                            <div>
                                <label for="weight" class="mt-2 block text-sm font-medium text-gray-700">ওজন (পাউন্ড)</label>
                                <input disabled value ="{{$profile->weight}}" placeholder="পাউন্ডে ওজন লিখুন (সংখ্যা) " type="text" name="weight" id="weight" class="mt-1 p-2 w-full border rounded-md" required>
                            </div>

                            <div>
                                <label for="present_address" class="mt-2 block text-sm font-medium text-gray-700">বর্তমান ঠিকানা (ইউনিট)</label>
                                <input disabled value ="{{$profile->present_address}}" placeholder="এখানে বর্তমান ঠিকানা/ইউনিট লিখুন   " type="text" name="present_address" id="present_address" class="mt-1 p-2 w-full border rounded-md" required>
                            </div>

                            <div>
                                <label for="permanent_address" class="mt-2 block text-sm font-medium text-gray-700">স্থায়ী ঠিকানা</label>
                                
                                <input disabled  value="{{$profile->vil}}" placeholder="গ্রাম/বাড়ি" type="text" name="vil" id="vil" class="mt-1 p-2 w-full border rounded-md" required>
                                <input disabled value ="{{$profile->union}}"  placeholder="ইউনিয়ন/পৌরসভা/ওয়ার্ড" type="text" name="union" id="union" class="mt-1 p-2 w-full border rounded-md" required>
                                <input disabled value ="{{$profile->upazila}}" placeholder="থানা/উপজেলা" type="text" name="upazila" id="upazila" class="mt-1 p-2 w-full border rounded-md" required>
                                <input disabled value ="{{$profile->po}}" placeholder="পোস্ট-অফিস" type="text" name="po" id="po" class="mt-1 p-2 w-full border rounded-md" required>
                                <input disabled value ="{{$profile->district}}"placeholder="জেলা" type="text" name="district" id="district" class="mt-1 p-2 w-full border rounded-md" required>
                            ...
                            <div>
                                ...
                                <p class="mt-2 block text-sm font-medium text-gray-700">
                                    চাকুরীর বয়স: {{$serviceAge->y}} বছর {{$serviceAge->m}} মাস {{$serviceAge->d}} দিন
                                </p>
                            </div>
                            
                            <div>
                                <label for="unit_join_date" class="mt-2 block text-sm font-medium text-gray-700"> ইউনিটে যোগদানের তারিখ</label>
                                <input disabled value="{{$unitJoinDate}}" name="unit_join_date" id="unit_join_date" datepicker datepicker-autohide datepicker-format="dd/mm/yyyy" type="text" class="mt-1 p-2 w-full border rounded-md bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full ps-10 p-2.5  dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="তারিখ নির্বাচন করুন" required autocomplete="off">
                                @php
                                // Correctly parse the date string using the format "d/m/Y"
                                $unitJoinDate = DateTime::createFromFormat('d/m/Y', $unitJoinDate);
                                $today = new DateTime('today');
                                $timeSinceJoined = $unitJoinDate->diff($today);
                            @endphp
                            
                            <p class="mt-2 block text-sm font-medium text-gray-700">
                                যোগদান হল: {{$timeSinceJoined->y}} বছর {{$timeSinceJoined->m}} মাস {{$timeSinceJoined->d}} দিন আগে
                            </p>
                            ...
                            <div>
                                ...
                                <input disabled value="{{$retirementDate}}" name="retirement_date" id="retirement_date" datepicker  datepicker-autohide datepicker-format="dd/mm/yyyy" type="text" class="mt-1 p-2 w-full border rounded-md bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full ps-10 p-2.5  dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"  required autocomplete="off">
                            
                                @php
                                    // Correctly parse the retirement date string using the format "d/m/Y"
                                    $retirementDate = DateTime::createFromFormat('d/m/Y', $retirementDate);
                                    $today = new DateTime('today');
                                    $serviceYearsLeft = $today->diff($retirementDate)->y;
                                @endphp

                                <p class="mt-2 block text-sm font-medium text-gray-700">
                                    চাকুরী বাকি: {{$serviceYearsLeft}} বছর
                                </p>
                            </div>

                            <div>
                                <label for="punishment" class="block text-sm font-medium text-gray-700">শাস্তির বিবরণ</label>
                                <input disabled placeholder="শাস্তির বিবরণ দিন (যদি থাকে)" value ="{{$profile->punishment}}" type="text" name="punishment" id="punishment" class="mt-1 p-2 w-full border rounded-md">
                            </div>
